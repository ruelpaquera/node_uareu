...
void Enrollment(DPFPDD_DEV hReader, int dpi){
	...
	while(!bStop){
		...
		while(!bDone){
			...
			if(bFirst){
				bFirst = 0;
				if(0 != CaptureFinger("any finger", hReader, dpi, DPFJ_FMD_ANSI_378_2004, &pFmd, &nFmdSize)){
					bStop = 1;
					break;
				}
			}
			else{
				if(0 != CaptureFinger("the same finger", hReader, dpi,DPFJ_FMD_ANSI_378_2004, &pFmd, &nFmdSize)){
					bStop = 1;
					break;
				}
			}
